...
export default function Home() {
  ...
  const phrases = useTranslations('phrases');
  const locale = useLocale();

  // Get all phrase keys and convert to array
  const phrasesArray = Array.from({ length: 75 }, (_, i) => phrases(`${i}`));
  ...
  const fetchStats = async (index: number) => {
    try {
      const response = await fetch(`/api/phrases/stats?phraseIndex=${index}&locale=${locale}`);
  ...
  const handleVote = async (type: 'up' | 'down') => {
    console.log('Voting call:', { type, isVoting, currentPhraseIndex, locale });
    if (isVoting || currentPhraseIndex === null) {
      console.log('Voting blocked or index null');
      return;
    }
    setIsVoting(true);
    ...
    try {
      ...
      const response = await fetch('/api/phrases/vote', {
        method: 'POST',
      ...
      if (response.ok) {
        setUserVote(type);
        await fetchStats(currentPhraseIndex);
      } else {
  ...
  const getRandomPhrase = async () => {
    ...
    const newPhrase = phrasesArray[randomIndex];
    ...
    console.log('New phrase generated:', { randomIndex, newPhrase });
    setIsVisible(true);
    setCopied(false);
    setUserVote(null); // Reset vote for new phrase
    setVotes({ upvotes: 0, downvotes: 0 }); // Reset visually
    fetchStats(randomIndex);
    ...
    // Track activity in our database (guest or logged in)
    // if (userId) {
    try {
    ...
    try {
      await fetch('/api/activity/track', {
        ...
        body: JSON.stringify({
          ...
          data: {
            platform: 'whatsapp',
            phraseIndex: animationKey // Not perfect but gives an index
          }
        }),
      });
    } catch (err) {
    ...
    <>
      ...
      <script
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            "mainEntity": {
              "@type": "CreativeWork",
              "name": "Demotivational Phrases for Developers",
              "description": "Collection of funny and demotivational phrases about programming, software development and tech career",
              "genre": "Humor",
              "inLanguage": "pt-BR"
            }
          })
      ...
      <div className="min-h-screen bg-black text-white font-sans">
        <main className="flex min-h-screen flex-col items-center justify-center px-4 py-8 font-sans sm:px-6 sm:py-12">
          {/* Header content moved to separate Header component */}
          ...
          <header className="text-center mb-12 sm:mb-16">
            <div>
              <Image
                ...
                alt="Desmotiva Dev - Humor for developers and programmers"
                width={160}
                height={160}
            ...
            <small className='font-light px-3 py-1 bg-slate-600 rounded-xl text-xs sm:text-sm'>
              {t('phrasesCount')}
            </small>
          </header>
          ...
          <section className="flex flex-col items-center gap-6 sm:gap-8 max-w-4xl w-full">
            ...
            {isVisible && (
              <article
                key={animationKey}
                className={`bg-gray-900 border border-gray-800 rounded-lg p-4 sm:p-8 max-w-3xl w-full text-center mx-4 sm:mx-0 ${animationKey === 0 ? 'animate-fade-in' : 'animate-phrase-update'
                  }`}
              >
                ...
                <div className="flex items-center justify-center gap-6 mb-6">
                  <button
                    ...
                    disabled={isVoting}
                    className={`flex flex-col items-center gap-1 transition-all duration-200 ${userVote === 'up' ? 'text-green-500 scale-110' : 'text-gray-400 hover:text-green-400'
                      }`}
                    aria-label="Upvote"
                  >
                  ...
                  <button
                    ...
                    disabled={isVoting}
                    className={`flex flex-col items-center gap-1 transition-all duration-200 ${userVote === 'down' ? 'text-red-500 scale-110' : 'text-gray-400 hover:text-red-400'
                      }`}
                    aria-label="Downvote"
                  >
                    <svg className="w-8 h-8" fill={userVote === 'down' ? 'currentColor' : 'none'} stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-3 sm:gap-4 mb-2" role="group" aria-label="Share options">
                  <button
                  ...
                  <button
                    ...
                    aria-label={`Share on ${t('shareWhatsApp')}`}
                  >
                  ...
                  <button
                    ...
                    aria-label={`Share on ${t('shareTwitter')}`}
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  ...
                  <button
                    ...
                    aria-label={`Share on ${t('shareLinkedIn')}`}
                  >
                    <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true">
                  ...
                  <button
                    onClick={() => {
                      const url = `/api/og?phrase=${encodeURIComponent(currentPhrase)}`;
                      window.open(url, '_blank');
                    }}
                    className="flex items-center justify-center gap-2 px-4 py-2 bg-gradient-to-r from-purple-600 to-pink-600 hover:from-purple-700 hover:to-pink-700 rounded-lg transition-all duration-200 w-full sm:w-auto font-bold shadow-lg transform hover:scale-105"
                ...
                <small className='text-slate-400 mt-1'>{t('shareHashtag')}</small>
          ...
          <footer className="text-center px-4">
            ...
            <p className="text-gray-500 text-xs sm:text-sm leading-relaxed">
              {t('footer.madeWith')} <a href="https://twitter.com/deveprogramar" className="hover:text-gray-300 transition-colors">@deveprogramar</a> <br />
              {t('footer.subtitle')}
            </p>
          </footer>
        </main>
